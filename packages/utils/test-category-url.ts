import NaverSmartstoreCrawler from './src/crawler/naver-smartstore-crawler';
import fs from 'fs';
...
async function main() {
  ...
  if (!categoryUrl) {
    console.error('사용법: ts-node test-category-url.ts <카테고리_URL>');
    process.exit(1);
  }
  ...
  try {
    // 출력 경로
    const outputPath = './data/category-products.json';
